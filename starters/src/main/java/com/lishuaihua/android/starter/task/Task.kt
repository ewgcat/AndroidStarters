...
import com.lishuaihua.android.starter.util.Utils.compareTask
import com.lishuaihua.android.starter.log.LogTaskListener
import com.lishuaihua.android.starter.task.listener.TaskListener
import com.lishuaihua.android.starter.task.listener.TaskListenerBuilder
...
abstract class Task @JvmOverloads constructor(//mId,唯一存在
    ...
    internal lateinit var StartersRuntime: StartersRuntime

    internal fun bindRuntime(StartersRuntime: StartersRuntime) {
        this.StartersRuntime = StartersRuntime;
    }
    ...
    open fun start() {
        ...
        StartersRuntime.executeTask(this)
    }

    override fun run() {
        if (StartersRuntime.debuggable && Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) {
    ...
    fun toStart() {
        ...
        StartersRuntime.setStateInfo(this)
        if (StartersRuntime.debuggable) {
    ...
    fun toRunning() {
        ...
        StartersRuntime.setStateInfo(this)
        StartersRuntime.setThreadName(this, Thread.currentThread().name)
        if (StartersRuntime.debuggable) {
    ...
    fun notifyBehindTasks() {
        ...
        if (behindTasks.isNotEmpty()) {
            if (behindTasks.size > 1) {
                Collections.sort(behindTasks, StartersRuntime.taskComparator)
            }
    ...
    open fun release() {
        ...
        StartersRuntime.setStateInfo(this)
        StartersRuntime.removeStarterTask(id)
        StartersRuntime.getTaskRuntimeInfo(id)?.clearTask()
        dependTasks.clear()
        behindTasks.clear()
        if (StartersRuntime.debuggable) {
