...
class StarterThreadPool {
     ...
     * 相对比 Starters, AsyncTask 更强调的是在业务处理中，异步业务不应该使得cpu饱和，但是启动场景时间比较短，可以尽可能使用更多的cpu资源。
     * 但是，Starters支持锚点阻塞ui线程，后续可能还会有延迟的异步初始化任务，所以也不要完全饱和。
     */
    ...
    private val sThreadFactory: ThreadFactory = object : ThreadFactory {
        ...
        override fun newThread(r: Runnable): Thread {
            return Thread(r, "Starters Thread #" + mCount.getAndIncrement())
        }
    }
