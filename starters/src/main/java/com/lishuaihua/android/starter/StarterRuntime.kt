...
import com.lishuaihua.android.starter.log.Logger.w
import com.lishuaihua.android.starter.util.Utils.compareTask
import com.lishuaihua.android.starter.task.Task
import com.lishuaihua.android.starter.task.TaskRuntimeInfo
import java.util.*
import java.util.concurrent.*

/**
 * Starters 框架 runtime 信息管理
 * 包含：
 * application 锚点管理
 ...
 * 运行时 Task 信息收集
 */
class StartersRuntime {

    //线程池，支持自定义
    private val pool: StarterThreadPool
    ...
    @Volatile
    internal var StarterTaskIds: MutableSet<String> = mutableSetOf()
    ...
    constructor(executor: ExecutorService? = null) {
        pool = StarterThreadPool(executor)
    }
    ...
    @Synchronized
    internal fun addStarterTasks(ids: Set<String>) {
        if (ids.isNotEmpty()) {
            StarterTaskIds.addAll(ids)
        }
    }

    @Synchronized
    internal fun removeStarterTask(id: String) {
        if (!TextUtils.isEmpty(id)) {
            StarterTaskIds.remove(id)
        }
    }

    internal fun hasStarterTasks(): Boolean {
        return StarterTaskIds.isNotEmpty()
    }
    ...
    internal fun traversalDependenciesAndInit(task: Task) {
        ...
        val iterator = StarterTaskIds.iterator()
        while (iterator.hasNext()) {
            val taskId = iterator.next()
            if (!hasTaskRuntimeInfo(taskId)) {
                if (debuggable) {
                    w(Constants.StarterS_INFO_TAG, "Starter \"$taskId\" no found !")
                }
                iterator.remove()
            } else {
    ...
    private fun traversalDependenciesAndInit(task: Task, traversalVisitor: LinkedHashSet<Task>) {
        ...
        if (taskRuntimeInfo == null) {
            ...
            if (StarterTaskIds.contains(task.id)) {
                info.isStarter = true
            }
